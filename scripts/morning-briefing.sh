...
- Pre-compaction persistence: ✅ CONFIGURED
- Auto-scheduling: ⏳ RUNNING (via cron)
...
$(if [ "$BUILD_PROPOSALS" -gt 0 ]; then
    ...
    for file in "$VAULT_PATH/03-patterns"/*.md; do
        if [ -f "$file" ] && grep -q "Status: Proposed" "$file" 2>/dev/null; then
            ...
            echo "- **$TITLE** — [[$(basename "$file" .md)|View]]"
        fi
    done
    echo ""
...
if [ -n "${TELEGRAM_BOT_TOKEN:-}" ] && [ -n "${TELEGRAM_CHAT_ID:-}" ]; then
    curl -s -X POST "https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/sendMessage" \
        ...
        -d "text=📊 Morning Briefing — $DATE%0A%0ASources: $SOURCES_CHECKED | Patterns: $PATTERNS_EXTRACTED | Proposals: $BUILD_PROPOSALS%0A%0ASee vault: 01-daily/$DATE-morning-briefing.md" \
